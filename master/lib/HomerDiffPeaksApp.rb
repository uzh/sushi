...
EOS
    ...
    @params['refBuildHOMER'] = ['hg38', 'mm10', 'mm39']
    @params['refBuildHOMER', 'description'] = 'The current supported genomes from HOMER. More are available upon request.'
    ...
    @params['balanced', 'description'] = 'Do not force the use of normalization factors to match total mapped reads.  This can be useful when analyzing differential peaks between similar data (e.g., H3K27ac) where we expect similar levels in all experiments. Applying this allows the data to essentially be quantile normalized during the differential calculation.'
