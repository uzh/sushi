...
class SpaceRangerDevApp <  SushiFabric::SushiApp
  def initialize
    super
    @employee = true
...
This wrapper runs <a href='https://support.10xgenomics.com/spatial-gene-expression/software/pipelines/latest/using/count',>space ranger count</a> in Single-library analysis mode.
    ...
    @params['refFeatureFile', "context"] = "SpaceRangerDev"
    @params['featureLevel'] = 'gene'
    @params['featureLevel', "context"] = "SpaceRangerDev"
  ...
  def next_dataset
    ...
    dataset = {
        'Name'=>@dataset['Name'],
        'SpaceRangerSummary [Link]'=>File.join(report_dir, 'web_summary.html'),
        'Species'=>@dataset['Species'],
        'refBuild'=>@params['refBuild'],
        'SpaceRangerDir [File]'=>report_dir,
        'Read Count'=>@dataset['Read Count'],
        'Image'=>@dataset['Image'],
        'SummedCount [Link]'=>File.join(report_dir, "#{@dataset['Name']}-summedCounts.txt"),
        'refFeatureFile'=>@params['refFeatureFile'],
        'featureLevel'=>@params['featureLevel'],
        'transcriptTypes'=>@params['transcriptTypes'],
      }.merge(extract_columns(@inherit_tags))
    if @dataset['Slide'] && @dataset['Slide'].start_with?('H')
      dataset['BinnedOutput'] = File.join(report_dir, 'binned_outputs/square_002um')
    end
  ...
  def commands
    ...
    command << run_RApp("EzAppSpaceRangerDev")
  end
end
