...
#      end
  ...
  def main(mock=false)
    ...
    if methods_script_path && !mock
      methods_job = Job.new
      methods_job.script_path      = File.join(@gstore_script_dir, File.basename(methods_script_path))
      methods_job.next_dataset_id  = @next_dataset_id
      methods_job.status           = "WAITING_FOR_METHODS"
      methods_job.user             = (@user || "sushi_lover")
      methods_job.input_dataset_id = dataset.id if dataset
      methods_job.save
    end
