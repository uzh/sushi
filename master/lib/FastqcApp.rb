...
#    @params['libQuantPlots', 'description'] = "make plot comparing library quantifications with read numbers"
 ...
 def set_default_parameters
  ...
  def next_dataset
    ...
    report_link = File.join(report_file, 'fastqc.html')
    reportMultiQC_link = File.join(report_file, 'multiqc_report.html')
    if @params['showNativeReports']
      ds = {'Name'=>@params['name'],
        'MultiQC [Link]'=>File.join(multiqc_file, 'multiqc_report.html'), ## that's the default name generated by the multiqc tool
        'FastQC [Link]'=>File.join(fastqc_file, 'fastqc.html'),
        'MultiQC [File]'=>multiqc_file,
        'FastQC [File]'=>fastqc_file
      }
    else
      ds = {'Name'=>@params['name'],
        'MultiQC [Link]'=>File.join(multiqc_file, 'multiqc_report.html'),
        'MultiQC [File]'=>multiqc_file,
        'FastQC [File]'=>fastqc_file
      }
    end
