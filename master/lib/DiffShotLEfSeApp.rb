...
class DiffShotLEfSeApp < SushiFabric::SushiApp
  def initialize
    super
    @name = 'DiffShotLEfSe'
...
EOS
    ...
    @params['daMethod'] = ['LEfSe']
    @params['daMethod', 'description'] = 'DA method run by this app. Hardcoded; use the DiffShotALDEx / DiffShotANCOMBC / DiffShotMaAsLin3 apps for covariate-adjusted methods.'
    ...
    @params['refGroup'] = ''
    @params['refGroup', 'description'] = 'Value of `grouping` used as the reference level.'

    # NOTE: no daCovariates field - LEfSe ignores covariates by design.
