...
class ApiController < ApplicationController
  ...
  def index
    if not request.method.eql? "POST"
      render :text => "Please POST data in JSON format.", :status => :bad_request
      return
    end
    ...
    begin
      ...
      render :text => "Error reading JSON data.", :status => :bad_request
      return
    end
    
    if not json.has_key? "project"
      render :text => "Please provide a project number: 'project'", :status => :bad_request
      return
    end
    
    project_id = json["project"].to_i
    if project_id == 0
      render :text => "Bad project number!", :status => :bad_request
      return
    end
    
    if not json.has_key? "name"
      render :text => "Please provide a dataset name: 'name'", :status => :bad_request
      return
    end
    dataset_name = json["name"]
    
    if not json.has_key? "path"
      render :text => "Please provide a dataset file: 'path'", :status => :bad_request
      return
    end
    ...
    begin
      ...
      render :text => "Cannot read the dataset file: "+dataset_file_path, :status => :bad_request
      return
    end
    
    if rows.length < 2
      render :text => "Dataset must contain one header line and at least one sample line.", :status => :bad_request
      return
    end
    ...
    if ds_id == 0
      render :text => "Cannot save the dataset!", :status => :internal_server_error
      return
    end
