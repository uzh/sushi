...
SUSHI can Support Users for SHell script Integration defined as a recursive acronym, but some one might say, SUSHI is a Super Ultra Special Hyper Incredible system!! Anyway, it is a sort of pipeline framework developed in Ruby on Rails in order to share useful pipeline scripts. An instance is now running at FGCZ to process the next genration sequence data for over 1000 projects. There have been some similar frameworks published such as Galaxy, Chipstar, and GeneProof. It is noteworthy that SUSHI is developed in Ruby on Rails, so that updating and installing is in quite simple manner.
...
ezRun package bundles a sort of wrapper applications to call a thrid party application. For example, FastQCApp calles fastqc command. In other words, FastQCApp does not work without installation of FastQC command. All the application paths are defined in EZ_GLOBAL_VARIABLES.txt. Please check the applications defined in EZ_GLOBAL_VARIABLES.txt one by one and set the correct path installed in your environement. If an appropriate application is not found, the corresponding SUSHI application will stop with an error, though the application button appears.
