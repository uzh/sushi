...
SUSHI is defined as an acronym of Supporting User for SHell script Integration, but some one might say, it is a Super Ultra Special Hyper Incredible system. Namely, it is a sort of pipeline framework in order to share useful pipeline script developed on Ruby on Rails. An instance is working at FGCZ in order to process the next genration sequence data for over 1000 projects. There have been some similar frameworks published such as Galaxy, Chipstar, and GeneProof. It is noteworthy that SUSHI is developed in Ruby on Rails, so that updating and installing is in quite simple manner.

== Getting Started

Just access and play around with the demo SUSHI server: fgcz-sushi-demo.uzh.ch. Basic operation is described here: http://fgcz-sushi.uzh.ch/usage.html 
...
Note
* The current version fo SUSHI does not work in Ruby on Rails version 4
